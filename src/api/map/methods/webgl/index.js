...
class WebglPoint {
  ...
  AddFn(data) {
    ...
    for (let k = -1; k < 2; k++) {
      let coor = transform([DF.lon + k * 360, DF.lat + k * 360], 'EPSG:4326', 'EPSG:3857');
      let geom = new Point(coor);
      let f = new Feature(Object.assign({ geometry: geom, center: coor }, DF));
      data[this.id] && f.setId(data[this.id] + '-' + k);
      f.set('geom', geom);
  ...
  replaceship(item) {
    for (let k = -1; k < 2; k++) {
      let f = this.Layer.getSource().getFeatureById(item[this.id] + '-' + k);
...
/*
    ...
    id唯一值字段名
*/
function DrawWebglPoint(map, layer, dataList, DefiningFeature, addType = false, id = 'id') {
  new WebglPoint(map, layer, DefiningFeature, id).AddShip(dataList, addType);
}
export default { DrawWebglPoint };

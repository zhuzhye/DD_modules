...
function fifterData(data, map, __minPix, __minAngle, allpoint) {
  ...
  var it0 = transform([Number(start.lon / 60 / 10000), Number(start.lat / 60 / 10000)], 'EPSG:4326', 'EPSG:3857');

  for (var i = 1; i <= data.length - 2; i++) {
    var item = data[i];
    // console.log([item.lon / 60 / 10000, item.lat / 60 / 10000]);
    var it = transform([Number(item.lon / 60 / 10000), Number(item.lat / 60 / 10000)], 'EPSG:4326', 'EPSG:3857');
    var pix = map.getPixelFromCoordinate(it);
    var pix0 = map.getPixelFromCoordinate(it0);
    if (Math.abs(pix[0] - pix0[0]) > __minPix || Math.abs(pix[1] - pix0[1]) > __minPix) {
      // console.log("加入了re");
      re.push(item);
      it0 = it;
    } else {

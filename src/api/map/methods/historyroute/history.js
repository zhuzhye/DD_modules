...
import { connectline, pathStyle, pathSEStyle } from '../../layers/historyrouteLayer/style';
...
class addhistorymethods {
  ...
  addhistory(dataList, map, minpx, mimangle, allpoint) {
    ...
    for (let i = 0; i < dataFifterList.length; i++) {
      let coor = transform(
        [dataFifterList[i].lon / 60 / 10000, dataFifterList[i].lat / 60 / 10000],
        'EPSG:4326',
        'EPSG:3857',
      );
      Coor.push(coor);
    ...
    for (let i = 0; i < dataFifterList.length; i++) {
      ...
      var center = {};
      let pix = map.getPixelFromCoordinate(FeaturePoint.getGeometry().getCoordinates());
      ...
      if (re.canDraw) {
        ...
        if (i > 0 && i < Coor.length - 1) {
          ...
          FeaturePoint.setStyle(pathStyle(dataFifterList[i], i + 1, dataFifterList[i + 1], re));
        }
        this.historypointLayer.getSource().addFeature(FeaturePoint);
        if (i % 2 == 0) {
        ...
        if (i > 0 && i < Coor.length - 1) {
          FeaturePoint.setStyle(pathStyle(dataFifterList[i], i + 1, dataFifterList[i + 1]));
          this.historypointLayer.getSource().addFeature(FeaturePoint);
        }
      }
      if (i == 0) {
      ...
      if (i == dataFifterList.length - 1) {
        ...
        endF.setStyle(pathSEStyle('end', re, dataFifterList[dataFifterList.length - 1]));
  ...
  calcLablePos(center, size, id) {
    ...
    if (!hasLable) {
      ...
      for (let i = pos.l; i < pos.l + pos.length; i++) {

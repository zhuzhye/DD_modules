...
measuremethods.createMeasureTooltip = function (map) {
  if (measuretarget.measureTooltipElement) {
    measuretarget.measureTooltipElement.parentNode.removeChild(measuretarget.measureTooltipElement);
  }
...
function CleanLine(deleteid, DrawLineLayer, map) {
  ...
  map
    ...
    .getArray()
    .forEach((item) => {
      if (item.values_.element && item.values_.element.getAttribute('id') == deleteid) {
